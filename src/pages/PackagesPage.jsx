    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-4 left-4">
          <p className="text-saffron-300 text-xs tracking-wider uppercase font-body mb-0.5">
            {pkg.destination}
          </p>
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <ul className="space-y-1.5 mb-4">
          {pkg.highlights
            .slice(0, expanded ? pkg.highlights.length : 3)
            .map((h, i) => (
              <li
                key={i}
                className="flex items-start gap-2 text-sm font-body text-earth-600"
              >
        ...
        <div className="flex flex-wrap gap-1.5 mb-5">
          {pkg.inclusions.slice(0, 4).map((inc, i) => (
            <span
              key={i}
              className="text-xs bg-jade-500/10 text-jade-600 px-2.5 py-1 rounded-full font-body"
            >
              ✓ {inc}
            </span>
          ))}
        ...
        <div className="flex items-end justify-between pt-4 border-t border-earth-100 mt-auto">
          <div>
            {pkg.originalPrice && (
              <p className="text-earth-400 text-xs line-through font-body">
                {pkg.originalPrice} / person
              </p>
            )}
            <p className="text-saffron-600 font-display text-2xl font-semibold">
              {pkg.price}
            </p>
      ...
      >
        ...
        <div className="relative h-60 flex-shrink-0">
          ...
          <div className="absolute bottom-4 left-5">
            <p className="text-saffron-300 text-xs uppercase tracking-wider font-body mb-0.5">
              {pkg.destination}
            </p>
            <h2 className="text-white font-display text-2xl font-medium">
              {pkg.name}
            </h2>
          </div>
        </div>
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="flex items-center justify-between mb-6 pb-6 border-b border-earth-100">
            <div>
              {pkg.originalPrice && (
                <p className="text-earth-400 text-xs line-through mb-0.5">
                  {pkg.originalPrice} / person
                </p>
              )}
              <p className="text-saffron-600 font-display text-3xl font-semibold">
                {pkg.price}
              </p>
            ...
            <div className="text-right space-y-1">
              ...
              <div className="flex items-center justify-end gap-1 mt-1">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`w-3.5 h-3.5 ${i < Math.floor(pkg.rating) ? "text-yellow-400 fill-yellow-400" : "text-earth-200"}`}
                  />
                ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-8">
            <div>
              <h3 className="font-body font-semibold text-charcoal mb-3 text-sm">
                ✅ What's Included
              </h3>
              <ul className="space-y-1.5">
                {pkg.inclusions.map((inc, i) => (
                  <li
                    key={i}
                    className="flex items-center gap-2 text-xs font-body text-earth-600"
                  >
            ...
            <div>
              ...
              <ul className="space-y-1.5">
                {pkg.exclusions.map((exc, i) => (
                  <li
                    key={i}
                    className="flex items-center gap-2 text-xs font-body text-earth-400"
                  >
          ...
          <p className="text-center text-earth-400 font-body text-xs mt-4">
            💰 Best price guaranteed • 🔄 Free cancellation within 48 hrs • 🛡
            Fully insured
          </p>
        </div>
      </motion.div>
...
export default function PackagesPage() {
  ...
  const filtered = useMemo(
    () =>
      activeFilter === "all"
        ? packages
        : packages.filter((p) => p.category.includes(activeFilter)),
    [activeFilter],
  );
    ...
    <>
      ...
      <section className="relative pt-36 pb-24 bg-black overflow-hidden">
        ...
        <div className="relative z-10 container-custom text-center max-w-3xl mx-auto">
          ...
          >
            {packages.length} handcrafted packages with premium stays, guided
            tours, and zero hidden costs. Click any card to see the full
            itinerary.
          </motion.p>
          ...
          >
            {[
              ...
              <span
                key={t}
                className="glass text-white/80 text-sm font-body px-4 py-2 rounded-full"
              >
                {t}
              </span>
            ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center text-white">
            {[
              ...
              },
              {
                ...
                sub: "Custom plan in 24 hrs",
              },
              {
                icon: "🔄",
                title: "Easy Changes",
                sub: "Flexible modifications",
              },
              {
                icon: "🌟",
                title: "98% Happy",
                sub: "12,000+ satisfied guests",
              },
            ].map((item) => (
              <div key={item.title}>
                <div className="text-3xl mb-2">{item.icon}</div>
                <p className="font-body font-semibold text-white mb-0.5">
                  {item.title}
                </p>
